...
#  Description: The program asks a user to enter three integers, an ending index, an integer, 
#  and another integer to use for a comparison. It then examines only the elements in the array 
#  located from the index 0 to the entered ending index then checks if each of them is greater the 
#  smallest of the last two entered numbers and is less than the largest of the last two entered numbers, 
#  then if it is, then it changes each such element in the array within the range according to the calculation 
#  given in the C program numbers[j] = numbers[j]*num1 + num2.
###########################################################
